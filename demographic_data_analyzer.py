...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(
        (df[advanced_edu]['salary'] == '>50K').sum() /
        advanced_edu.sum() * 100, 1
    )
    ...
    # ── 7. % of min-hour workers earning >50K ─────────────────────────────────
    ...
    country_rich_percent = (country_salary / country_total * 100).round(1)

    highest_earning_country            = country_rich_percent.idxmax()
    highest_earning_country_percentage = country_rich_percent.max()
